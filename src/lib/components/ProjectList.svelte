	...
	onMount(() => {
		data.catItems.forEach((category, catIndex) => {
			...
			category.items.forEach((item: Post | SanityReference<Post>, postIndex: number): void => {
				(item as Post).index = catIndex ? data.categoriesLength[catIndex - 1] + postIndex : postIndex;
	...
	{#if posts}
		{#each list as item, index}
			<ListItem
				...
				{selHeight}
				{activeIndex}
				active={typeof item.content !== 'string' &&
					(item as Post).index === countStore.activeIndex}
			></ListItem>
		{/each}
	{/if}
	<!-- {#each data.catItems as category, catIndex}
		{#if !$page.params.slug && mounted}
			<ListItem {catIndex} item={category.title} {data}></ListItem>
		{/if}
		{#each category.items as item, itemIndex}
			{#if (!$page.params.slug || ($page.params.slug && id === (item as Post)._id)) && mounted}
				<ListItem {itemIndex} {catIndex} {item} {data}></ListItem>
			{/if}
		{/each}
	{/each} -->
</ul>
